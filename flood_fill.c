...
/*   Updated: 2026/01/12 16:14:35 by rchaumei         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (start_x < 0 || start_y < 0 || start_x > map.len_line
		|| start_y > map.num_line)
		return ;
	if (tmp[start_y][start_x] == '1' || tmp[start_y][start_x] == 'M')
		return ;
...
int	check_flood_fill(char **tmp)
{
	...
	while (tmp[i])
	{
		...
		while (tmp[i][j])
		{
			if (tmp[i][j] != '0' && tmp[i][j] != '1' && tmp[i][j] != 'M')
				return (1);
			j++;
		}
		i++;
